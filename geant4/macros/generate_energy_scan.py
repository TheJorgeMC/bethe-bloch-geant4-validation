...
# are encoded with 'p' instead of the decimal point: 0.001 MeV ->
# "dedx_0p001MeV_cut0p01mm". The /gun/energy command keeps the numeric
# value. Same reason there is no /control/foreach: each point needs the
# numeric energy AND the 'p'-tag (plus now a per-point thickness), and a
# single foreach alias cannot carry them all — points are written inline.
# run_point.mac remains only as a deprecated stub for the CMake
# configure_file() rule.
#
# Usage:
...
def main() -> None:
    ...
    for e in energies:
        ...
        macro = POINT_TEMPLATE.format(
            ...
            material=MATERIAL, size_xy=SIZE_XY, n_layers=N_LAYERS,
            cut=CUT, cut_tag=CUT_TAG, n_events=N_EVENTS)
